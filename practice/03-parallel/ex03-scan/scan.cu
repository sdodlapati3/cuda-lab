...
// =============================================================================
// TODO: Hillis-Steele inclusive scan (within block)
// =============================================================================
__global__ void scan_hillis_steele(float* out, const float* in, int n) {
    ...
    // TODO: Implement Hillis-Steele scan
...
// =============================================================================
// TODO: Blelloch exclusive scan (within block)
// =============================================================================
__global__ void scan_blelloch(float* out, const float* in, int n) {
    ...
    // TODO: Set last element to 0 for exclusive scan
...
int main() {
    const int N = BLOCK_SIZE;  // Single block for simplicity
    
    printf("Prefix Sum (Scan) Exercise\n");
    printf("==========================\n");
    ...
    cpu_scan_inclusive(h_ref, h_in, N);
    ...
    printf("Hillis-Steele: last element = %.0f (expected %.0f)\n", h_out[N-1], h_ref[N-1]);
    
    // Test Blelloch
    scan_blelloch<<<1, BLOCK_SIZE>>>(d_out, d_in, N);
    CHECK_CUDA(cudaMemcpy(h_out, d_out, N * sizeof(float), cudaMemcpyDeviceToHost));
    
    printf("Blelloch (exclusive): last element = %.0f (expected %.0f)\n", h_out[N-1], h_ref[N-2]);
