...
void runWithGraph(float* d_data, float* d_partial, int n, int blocks) {
    // TODO: Create CUDA Graph
    cudaGraph_t graph;
    cudaGraphExec_t graphExec;
    
    // TODO: Begin stream capture
    cudaStream_t stream;
    cudaStreamCreate(&stream);
    
    // TODO: Capture kernels into graph
    // cudaStreamBeginCapture(stream, cudaStreamCaptureModeGlobal);
    
    // TODO: Launch kernels on the stream (they will be captured)
    // initKernel<<<blocks, 256, 0, stream>>>(...);
    // squareKernel<<<blocks, 256, 0, stream>>>(...);
    // addKernel<<<blocks, 256, 0, stream>>>(...);
    // reduceKernel<<<blocks, 256, 0, stream>>>(...);
    
    // TODO: End capture
    // cudaStreamEndCapture(stream, &graph);
    
    // TODO: Create executable graph
    // cudaGraphInstantiate(&graphExec, graph, NULL, NULL, 0);
    
    // TODO: Launch the graph
...
int main() {
    ...
    // TODO: Benchmark with graph
    // cudaEventRecord(start);
    // for (int i = 0; i < ITERATIONS; i++) {
    //     runWithGraph(d_data, d_partial, N, blocks);
    // }
    // cudaEventRecord(stop);
    // cudaEventSynchronize(stop);
    // float msWith;
